...
app.use("/views/img", express.static(__dirname + "/views/img"));
...
http.listen(3000, function () {
    ...
    mongoClient.connect("mongodb://localhost:27017", {
        ...
        app.get("/allUpload", async function (request, result) {
            if (request.session.user) {
                var links = await database.collection("public_links").find().toArray();

                result.render("allUpload", {
                    "request": request,
                    "links": links
                });
        ...
        app.post("/about", async function (request, result) {
            var email = request.fields.email;
            ...
            await database.collection("feedback").insertOne({
                ...
                request.status = "success";
                request.message = "Feedback is successfully given.";
